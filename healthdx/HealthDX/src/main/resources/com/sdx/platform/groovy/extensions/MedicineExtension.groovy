...
class MedicineExtension  extends MedicineDevice implements Basics {
	...
	def buildFIO(HashMap dbref) {
		...
		Map nameMap  	       		    = FieldUtils.buildTF("Device Name","name", name, false);
		Map deviceIdMap           	    = FieldUtils.buildTF("Device Id","deviceId", deviceId, false);
		Map typeMap   	                = FieldUtils.buildETFF("Device Type","type", type, defaultTypes, false);
		Map manufacturerMap  	        = FieldUtils.buildTF("Manufacturer","manufacturer", manufacturer, false);
		Map serialNoMap                 = FieldUtils.buildTF("Serial No","serialNo", serialNo, false);
