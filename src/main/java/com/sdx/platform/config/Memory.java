...
import com.sdx.camel.DeviceDataPullRouter;
import com.sdx.eventHandling.AnonymousEventHandler;
...
public class Memory {
	...
	private static EPRuntime CEP_ENGINE = null;
	
	private static AnonymousEventHandler ANONYMOUS_HANDLER = new AnonymousEventHandler();
	
	private static LinkedHashMap<String, String> DEVICE_AUTH_CODES = null;
	
	private static CamelContext GLOBAL_CONTEXT	= null;
	...
	public static EPRuntime getCEP_ENGINE() {
		if (CEP_ENGINE == null) {
			Configuration config = new Configuration();
	        
			/*
			 * Map<String, Object> definition = new LinkedHashMap<String, Object>();
			 * definition.put("deviceID", String.class); definition.put("userID",
			 * String.class); config.getCommon().addEventType("SensorEventWindow",
			 * definition);
			 */
	        
	        // Get an runtime instance
	        CEP_ENGINE = EPRuntimeProvider.getDefaultRuntime(config);
	        System.out.println("CEP_ENGINE "+CEP_ENGINE.getRuntimePath().toString());
	        
	        //compileDeploy(CEP_ENGINE, "@JsonSchema(dynamic=true) @public @buseventtype create json schema AnonnyEvent(eventUUID string)");
	        //compileDeploy(CEP_ENGINE, "@JsonSchema(dynamic=true) @public @buseventtype create json schema SensorEvent()", "SensorPlayground");
	        
	        //CEP_ENGINE.getEPAdministrator().createEPL("@JsonSchema(dynamic=true) @public @buseventtype create json schema SensorEvent()");
		}
		return CEP_ENGINE;
	}

	public static LinkedHashMap<String, String> getDEVICE_AUTH_CODES() {
		if (DEVICE_AUTH_CODES == null) {
			DEVICE_AUTH_CODES = new LinkedHashMap<String, String>();
			DEVICE_AUTH_CODES.put("fitbitSivi", "Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiIyMkJIN0giLCJzdWIiOiI4QkpaQlkiLCJpc3MiOiJGaXRiaXQiLCJ0eXAiOiJhY2Nlc3NfdG9rZW4iLCJzY29wZXMiOiJ3aHIgd251dCB3cHJvIHdzbGUgd3dlaSB3c29jIHdzZXQgd2FjdCB3bG9jIiwiZXhwIjoxNjE1NDYyMzI4LCJpYXQiOjE1ODM5MjYzMjh9.W0rnQYrbLIwX28Sa8xCgsks-0ZDWJ3y5jf8eSaEv2nw");
		}
		return DEVICE_AUTH_CODES;
	}

	public static void setDEVICE_AUTH_CODES(LinkedHashMap<String, String> dEVICE_AUTH_CODES) {
		DEVICE_AUTH_CODES = dEVICE_AUTH_CODES;
	}

	public static AnonymousEventHandler getANONYMOUS_HANDLER() {
		return ANONYMOUS_HANDLER;
	}

	public static void setANONYMOUS_HANDLER(AnonymousEventHandler aNONYMOUS_HANDLER) {
		ANONYMOUS_HANDLER = aNONYMOUS_HANDLER;
	}

	public static CamelContext getGLOBAL_CONTEXT() {
		if (GLOBAL_CONTEXT == null) {
			GLOBAL_CONTEXT = new DefaultCamelContext();
			try {
				GLOBAL_CONTEXT.addRoutes(new DeviceDataPullRouter());
				System.out.println("Device Pull Router added");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public static void setGLOBAL_CONTEXT(CamelContext gLOBAL_CONTEXT) {
